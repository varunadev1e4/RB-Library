...
export default function BottomNav() {
  ...
  const navItems = [
    { to: '/',          label: 'Home',   icon: icons.home,   exact: true },
    { to: '/search',    label: 'Search', icon: icons.search  },
    { to: '/my-books',  label: 'Books',  icon: icons.book    },
    { to: '/profile',   label: 'Me',     icon: icons.user    },
    ...(isAdmin ? [{ to: '/admin', label: 'Admin', icon: icons.admin }] : []),
  ]

  return (
    /* bottom-nav class handles height + safe-area padding from CSS */
        ...
        >
          {({ isActive }) => (
            <>
              ...
              <span className={`text-[10px] font-sans font-medium leading-none ${isActive ? 'text-amber-400' : 'text-mahogany-400'}`}>
                {label}
              </span>
            </>
          )}
