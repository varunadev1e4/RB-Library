...
| **Offline support** | Service worker caches app shell + fonts |
| **Network-first API** | Supabase calls use network-first with 5-min cache |
| **Auto-update** | In-app banner when a new version is available |
| **Safe areas** | Handles notch/Dynamic Island/home indicator on iOS |
| **No zoom on input** | All inputs use 16px to prevent iOS keyboard zoom |
| **Pull-to-refresh** | Visual pull indicator on mobile |

### PWA Install (Android)
Chrome shows an automatic "Add to Home Screen" banner.
...
```bash
npm run build
# dist/ folder contains the complete PWA
```

The service worker (`dist/sw.js`) is auto-generated by vite-plugin-pwa with Workbox.
